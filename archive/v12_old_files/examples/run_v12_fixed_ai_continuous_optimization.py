...
from src.v12_ensemble_ai_model_fixed import V12EnsembleAIModel  # 使用修复版本
...
class V12FixedAIContinuousOptimizer:
    """V12修复版AI持续优化器"""
    ...
    def __init__(self):
        ...
        self.optimization_params = {
            ...
            'ofi_z_threshold': 1.2,            # 降低阈值
            'ai_confidence_threshold': 0.55,   # 降低阈值
            'min_signal_quality': 0.35,        # 降低阈值
        ...
        logger.info(f"V12修复版AI持续优化器初始化完成 - 设备: {self.device}")
    ...
    def run_single_optimization_cycle(self, cycle: int, seed: int = None) -> Dict:
        """运行单次优化循环"""
        try:
            logger.info(f"开始修复版AI优化循环 {cycle}...")
            ...
            logger.info(f"修复版AI优化循环 {cycle} 完成 - 交易数: {len(trades)}, 胜率: {performance.get('win_rate', 0):.2%}, PnL: {performance.get('total_pnl', 0):.4f}")
            
            return optimization_record
            
        except Exception as e:
            logger.error(f"修复版AI优化循环 {cycle} 失败: {e}")
            return {'cycle': cycle, 'error': str(e)}
    
    def run_continuous_optimization(self, num_cycles: int = 3) -> Dict:
        """运行持续优化"""
        try:
            logger.info(f"开始修复版AI持续优化 - 循环次数: {num_cycles}")
            ...
            results_file = f"backtest_results/v12_fixed_ai_continuous_optimization_{timestamp}.json"
            
            os.makedirs("backtest_results", exist_ok=True)
            with open(results_file, 'w', encoding='utf-8') as f:
                json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
            
            logger.info(f"修复版AI持续优化完成，结果已保存到: {results_file}")
            
            return summary
            
        except Exception as e:
            logger.error(f"修复版AI持续优化失败: {e}")
            return {'error': str(e)}

def main():
    """主函数"""
    try:
        logger.info("V12修复版AI持续优化系统启动")
        
        # 初始化优化器
        optimizer = V12FixedAIContinuousOptimizer()
        
        # 运行持续优化
        results = optimizer.run_continuous_optimization(num_cycles=3)
        
        if 'error' in results:
            logger.error(f"修复版AI持续优化失败: {results['error']}")
            return
        
        # 生成优化报告
        logger.info("生成修复版AI优化报告...")
        generate_fixed_ai_optimization_report(results)
        
        logger.info("V12修复版AI持续优化完成")
        
    except Exception as e:
        logger.error(f"修复版AI持续优化系统失败: {e}")
        raise

def generate_fixed_ai_optimization_report(results: Dict):
    """生成修复版AI优化报告"""
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        report_content = f"""# V12修复版AI持续优化报告
...
"""
        ...
        report_file = f"V12_FIXED_AI_CONTINUOUS_OPTIMIZATION_REPORT_{timestamp}.md"
        with open(report_file, 'w', encoding='utf-8') as f:
            f.write(report_content)
        
        logger.info(f"修复版AI优化报告已保存: {report_file}")
