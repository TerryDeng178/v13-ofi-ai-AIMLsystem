...
class BinanceOrderBookStream:
    ...
    def __init__(self, symbol='ethusdt', depth_levels=5):
        ...
        self.stats = {
            ...
            # 序列一致性统计（期货WS严格对齐 - 修正版2）
            'resyncs': 0,  # resync次数（保留字段，实际始终为0）
            'reconnects': 0,  # 重连次数
            'batch_span_sum': 0,  # batch_span累计（仅观测）
            'batch_span_max': 0,  # 最大batch_span
            'last_u': None,  # 上一次的u值（用于pu字段记录）
        }
