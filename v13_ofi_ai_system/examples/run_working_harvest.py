...
# -*- coding: utf-8 -*-
"""
工作版OFI+CVD数据采集脚本 (基于Task 1.2.4成功实现)
使用正确的Binance Futures WebSocket URL和消息解析
"""
...
try:
    ...
    print("警告: 无法导入RealOFICalculator，将使用简化OFI计算")
    OFI_AVAILABLE = False
...
class WorkingOFICVDHarvester:
    """工作版OFI+CVD数据采集器（基于Task 1.2.4成功实现）"""
    
    def __init__(self, symbols=None, run_hours=1, output_dir="data/ofi_cvd"):
        ...
        logger.info(f"初始化工作版采集器: {self.symbols}, 运行{run_hours}小时")
    ...
    def _parse_orderbook_message(self, message: str) -> Optional[Dict]:
        """解析订单簿消息（基于Task 1.2.4的成功实现）"""
        try:
            raw = json.loads(message)
            
            # 处理Binance Futures格式（基于Task 1.2.4的成功实现）
            if "data" in raw:
                data = raw["data"]
                bids = data.get("b", [])  # Binance使用 'b' 而不是 'bids'
                asks = data.get("a", [])  # Binance使用 'a' 而不是 'asks'
            else:
                bids = raw.get("bids", [])
                asks = raw.get("asks", [])
            ...
            return {
                'bids': topk_pad(bids, 5, reverse=True),
                'asks': topk_pad(asks, 5, reverse=False)
            }
    ...
    def _calculate_ofi(self, symbol: str, orderbook: Dict) -> Optional[Dict]:
        """计算OFI指标（使用Task 1.2.4的成功实现）"""
        try:
            if not orderbook or 'bids' not in orderbook or 'asks' not in orderbook:
                return None
            
            # 使用RealOFICalculator
            if symbol in self.ofi_calculators:
                result = self.ofi_calculators[symbol].update(orderbook['bids'], orderbook['asks'])
                if result:
                    return {
            ...
            ofi_z = ofi_value / 1000.0 if ofi_value != 0 else 0.0
    ...
    def _calculate_cvd(self, symbol: str, trade_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # 简化的Z-score计算
            cvd_z = delta / 100.0 if delta != 0 else 0.0
    ...
    async def _process_trade_data(self, symbol: str, trade_data: Dict):
        ...
        try:
            ...
            # 5. 检测事件
            events = self._detect_events(ofi_result, cvd_result, float(trade_data.get('price', 0)))
            for event in events:
                event_data = {
                    'ts_ms': current_time,
                    'symbol': symbol,
            ...
            async with websockets.connect(url, ping_interval=20) as websocket:
                ...
                async for message in websocket:
                    ...
                    try:
                        ...
                        # 每10条数据保存一次
                        if len(self.data_buffers['prices'][symbol]) >= 10:
                            for kind in ['prices', 'ofi', 'cvd', 'fusion', 'events']:
    ...
    async def connect_orderbook_stream(self, symbol: str):
        """连接订单簿流（使用Binance Futures - 基于Task 1.2.4成功实现）"""
        # 使用Task 1.2.4中成功的Binance Futures URL
        stream_name = f"{symbol.lower()}@depth5@100ms"
        url = f"wss://fstream.binance.com/ws/{stream_name}"
        
        logger.info(f"连接订单簿流: {symbol} (使用Binance Futures)")
    ...
    async def run(self):
        """运行工作版采集器"""
        logger.info("开始工作版数据采集（基于Task 1.2.4成功实现）...")
...
async def main():
    ...
    run_hours = int(os.getenv('RUN_HOURS', '1'))
    output_dir = os.getenv('OUTPUT_DIR', 'data/ofi_cvd')
    
    # 创建工作版采集器
    harvester = WorkingOFICVDHarvester(symbols, run_hours, output_dir)
