...
# -*- coding: utf-8 -*-
"""
修复版OFI+CVD数据采集脚本 (Task 1.3.1 v2)
修复订单簿流连接问题，确保完整数据采集
"""
...
class FixedOFICVDHarvester:
    ...
    def __init__(self, symbols=None, run_hours=1, output_dir="data/ofi_cvd"):
        ...
        logger.info(f"初始化修复版采集器: {self.symbols}, 运行{run_hours}小时")
    ...
    def _calculate_ofi(self, symbol: str, orderbook: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            ofi_value = 0.0
            for i in range(5):
                bid_qty = max(0, float(bids[i][1]))
                ask_qty = max(0, float(asks[i][1]))
                weight = weights[i]
...
async def main():
    ...
    # 创建修复版采集器
    harvester = FixedOFICVDHarvester(symbols, run_hours, output_dir)
