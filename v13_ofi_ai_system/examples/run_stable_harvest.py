...
# -*- coding: utf-8 -*-
"""
稳定版OFI+CVD数据采集脚本 (Task 1.3.1 v2)
...
class StableOFICVDHarvester:
    ...
    def __init__(self, symbols=None, run_hours=1, output_dir="data/ofi_cvd"):
        ...
        logger.info(f"初始化稳定版采集器: {self.symbols}, 运行{run_hours}小时")
            ...
            async with websockets.connect(url, ping_interval=20) as websocket:
                ...
                async for message in websocket:
                    ...
                    try:
                        ...
                        # 每20条数据保存一次
                        if len(self.data_buffers['prices'][symbol]) >= 20:
                            for kind in ['prices', 'ofi', 'cvd', 'fusion', 'events']:
...
async def main():
    ...
    # 创建稳定版采集器
    harvester = StableOFICVDHarvester(symbols, run_hours, output_dir)
