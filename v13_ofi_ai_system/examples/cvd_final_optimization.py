...
def create_test_config(mad_multiplier, scale_fast_weight, half_life_trades, winsor_limit, output_dir):
    """创建最终优化测试配置"""
    config_content = f"""# CVD Z-score微调优化配置 - 最终优化
# 基于超精细搜索结果进行最终微调
...
"""
    ...
    config_file = output_dir / f"final_{mad_multiplier}_{scale_fast_weight}_{half_life_trades}_{winsor_limit}.env"
...
def analyze_z_score_distribution(parquet_file):
    ...
    try:
        ...
        # 最终评分（更重视P(|Z|>2)）
        score = (p_z_gt_2 * 0.6 + p_z_gt_3 * 0.3 + min(p95_z/2.0, 1.0) * 0.1)
...
def main():
    parser = argparse.ArgumentParser(description="CVD Z-score微调优化 - 最终优化")
    parser.add_argument("--symbol", default="BTCUSDT", help="交易对")
    parser.add_argument("--duration", type=int, default=1200, help="每个测试的时长（秒）")
    parser.add_argument("--output-dir", type=Path, default=Path("data/cvd_final_optimization"), help="输出目录")
    ...
    # 基于最佳结果的微调参数范围
    mad_multipliers = [1.15, 1.20, 1.25]  # 围绕最佳值1.2微调
    scale_fast_weights = [0.10, 0.15, 0.20]  # 围绕最佳值0.15微调
    half_life_trades = [80, 100, 120]  # 围绕最佳值100微调
    winsor_limits = [2.0, 2.5, 3.0, 3.5]  # 测试不同截断值
    
    print("="*80)
    print("CVD Z-score微调优化 - 最终优化")
    print("="*80)
    ...
    if best_result:
        ...
        if p_z_gt_3_ok and p_z_gt_2_ok and median_ok and p95_ok:
            print(f"\n找到最终达标配置: {Path(best_result['config_file']).name}")
            print("所有指标均达到最终标准！")
        else:
            print(f"\n最佳配置仍需优化，但已取得重大进展")
    ...
    results_file = args.output_dir / f"final_optimization_results_{timestamp}.json"
