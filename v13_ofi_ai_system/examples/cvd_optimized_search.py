...
def create_test_config(mad_multiplier, scale_fast_weight, half_life_trades, winsor_limit, output_dir):
    """创建基于经验的优化测试配置"""
    config_content = f"""# CVD Z-score微调优化配置 - 基于社区经验
# 参考网络最佳实践进行参数优化
...
"""
    ...
    config_file = output_dir / f"opt_{mad_multiplier}_{scale_fast_weight}_{half_life_trades}_{winsor_limit}.env"
...
def analyze_z_score_distribution(parquet_file):
    ...
    try:
        ...
        # 基于经验的综合评分（越小越好）
        # P(|Z|>3)权重最高，P(|Z|>2)次之，P95(|Z|)再次
        score = (p_z_gt_3 * 0.5 + p_z_gt_2 * 0.3 + min(p95_z/5.0, 1.0) * 0.2)
...
def main():
    parser = argparse.ArgumentParser(description="CVD Z-score微调优化 - 基于网络经验")
    parser.add_argument("--symbol", default="BTCUSDT", help="交易对")
    parser.add_argument("--duration", type=int, default=600, help="每个测试的时长（秒）")
    parser.add_argument("--output-dir", type=Path, default=Path("data/cvd_optimized_search"), help="输出目录")
    parser.add_argument("--max-tests", type=int, default=15, help="最大测试数量")
    ...
    # 基于网络经验的优化参数范围
    mad_multipliers = [1.35, 1.40, 1.45, 1.50, 1.55]  # 扩大MAD范围
    scale_fast_weights = [0.20, 0.25, 0.30, 0.35, 0.40]  # 更保守的权重
    half_life_trades = [150, 200, 250, 300, 350]  # 更短的半衰期
    winsor_limits = [5.0, 6.0, 8.0, 10.0]  # 更严格的截断
    
    print("="*80)
    print("CVD Z-score微调优化 - 基于网络经验")
    print("="*80)
    ...
    if len(combinations) > args.max_tests:
        ...
        priority_combinations = [
            (1.45, 0.30, 250, 6.0),  # 经验推荐组合1
            (1.40, 0.25, 200, 5.0),  # 经验推荐组合2
            (1.50, 0.35, 300, 8.0),  # 经验推荐组合3
            (1.35, 0.20, 150, 5.0),  # 保守组合
            (1.55, 0.40, 350, 10.0), # 激进组合
        ]
    ...
    if best_result:
        ...
        if p_z_gt_3_ok and p_z_gt_2_ok and median_ok and p95_ok:
            ...
            print("所有指标均达到基于经验的标准！")
        else:
            print(f"\n最佳配置仍需优化，建议调整参数范围")
    
    # 保存结果
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_file = args.output_dir / f"optimized_search_results_{timestamp}.json"
