...
class SuccessOFICVDHarvester:
    ...
    def _parse_orderbook_message(self, message: str) -> Optional[Dict]:
        """解析订单簿消息（基于Task 1.2.5成功实现）"""
        try:
            raw = json.loads(message)
            
            # 处理Binance Futures格式（基于Task 1.2.5成功实现）
            if "data" in raw:
                data = raw["data"]
    ...
    def _calculate_ofi(self, symbol: str, orderbook: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if symbol in self.ofi_calculators:
                result = self.ofi_calculators[symbol].update_with_snapshot(orderbook['bids'], orderbook['asks'])
                if result:
                    return {
    ...
    def _calculate_fusion(self, ofi_result: Dict, cvd_result: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # 使用sigmoid函数进行概率校准
            import math
            k = 1.0  # 校准参数，可根据历史数据优化
            proba = 1 / (1 + math.exp(-k * fusion_raw))
            
            # 计算信号一致性（方向一致性）
            consistency = 0.0
            if (ofi_z > 0 and cvd_z > 0) or (ofi_z < 0 and cvd_z < 0):
                consistency = min(abs(ofi_z), abs(cvd_z)) / max(abs(ofi_z), abs(cvd_z), 1e-8)
    ...
    def _detect_events(self, symbol: str, ofi_result: Dict, cvd_result: Dict, price: float) -> List[Dict]:
        """检测事件（背离/枢轴/异常）- 基于Task 1.2.12规范"""
        events = []
    ...
    async def _process_trade_data(self, symbol: str, trade_data: Dict):
        ...
        try:
            ...
            # 5. 检测事件
            events = self._detect_events(symbol, ofi_result, cvd_result, float(trade_data.get('price', 0)))
            for event in events:
                event_data = {
                    'ts_ms': current_time,
                    'symbol': symbol,
    ...
    async def connect_orderbook_stream(self, symbol: str):
        """连接订单簿流（使用Task 1.2.5成功的URL格式）"""
        # 使用Task 1.2.5中成功的Binance Futures URL格式
        url = f"wss://fstream.binancefuture.com/stream?streams={symbol.lower()}@depth@100ms"
        
        logger.info(f"连接订单簿流: {symbol} (使用Task 1.2.5成功URL格式)")
...
async def main():
    ...
    run_hours = float(os.getenv('RUN_HOURS', '1'))
