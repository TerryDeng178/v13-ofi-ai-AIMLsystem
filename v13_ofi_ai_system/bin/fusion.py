...
def main():
    parser = argparse.ArgumentParser(description='FUSION融合器组件')
    ...
    if args.print_effective:
        print_component_effective_config(cfg, 'fusion', verbose=args.verbose)
    
    # 干运行模式
    if args.dry_run_config:
        print("[DRY-RUN] FUSION组件配置验证通过")
        return 0
    
    # 实际运行组件（TODO: 添加自测或演示逻辑）
    print("[INFO] FUSION组件初始化成功（库式调用模式）")
    print("[提示] 请通过CoreAlgorithm或其他上层组件调用OFI_CVD_Fusion类")
