...
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐             │
│  │  配置管理层      │    │  真实数据输入层  │    │  核心计算层      │             │
│  │ (Config Mgmt)  │    │ (Real Data)    │    │ (Core Calc)    │             │
│  │                │    │                │    │                │             │
│  │ • 统一配置系统   │    │ • 币安WebSocket │    │ • 真实OFI计算   │             │
│  │ • 环境变量覆盖   │───▶│ • 订单簿流(5档) │───▶│ • 真实CVD计算   │             │
│  │ • 动态模式切换   │    │ • 成交流(aggTrade)│  │ • 融合指标      │             │
│  │ • 原子热更新    │    │ • 实时序列监控  │    │ • 背离检测      │             │
│  │ • 监控告警      │    │ • 延迟追踪      │    │ • Z-score标准化 │             │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘             │
│           │                        │                        │                │
│           │                        │                        │                │
│           ▼                        ▼                        ▼                │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐             │
│  │  数据存储层      │    │  信号处理层      │    │  AI模型层        │             │
│  │ (Data Storage) │    │ (Signal Proc)  │    │ (AI Models)    │             │
│  │                │    │                │    │                │             │
│  │ • NDJSON原始流  │    │ • OFI信号生成   │    │ • OFI专家模型   │             │
│  │ • Parquet分析库 │───▶│ • CVD信号生成   │───▶│ • LSTM模型      │             │
│  │ • 内存缓存      │    │ • 融合信号      │    │ • Transformer   │             │
│  │ • 延迟追踪      │    │ • 背离信号      │    │ • CNN模型       │             │
│  │ • 序列一致性    │    │ • 信号验证      │    │ • 集成模型      │             │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘             │
│           │                        │                        │                │
...
│  │                │    │                │    │                │             │
│  │ • 测试网交易     │    │ • Prometheus指标│    │ • 动态模式切换   │             │
│  │ • 简单策略      │    │ • Grafana仪表盘 │    │ • 参数热更新     │             │
│  │ • 风险管理      │    │ • 告警规则      │    │ • 性能调优      │             │
│  │ • 成本控制      │    │ • 日志分析      │    │ • 资源监控      │             │
│  │ • 动态仓位      │    │ • 健康检查      │    │ • 负载均衡      │             │
│  └─────────────────┘    └─────────────────┘    └─────────────────┘             │
