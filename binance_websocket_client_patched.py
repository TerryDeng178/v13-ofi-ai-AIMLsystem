...
class BinanceOrderBookStream:
    def __init__(self,
        ...
        self.stats: Dict[str, Any] = dict(
            ...
            last_metrics_ts = time.monotonic(),
        )
    ...
    def _maybe_emit(self, ts_recv: int):
        # print summary
        if (time.monotonic() - self.stats["last_summary_ts"]) >= self.print_interval:
            self.stats["last_summary_ts"] = time.monotonic()
            self.print_statistics()
        # save metrics json every 10s
        if (time.monotonic() - self.stats["last_metrics_ts"]) >= 10.0:
            self.stats["last_metrics_ts"] = time.monotonic()
