...
class V10RealtimeSimulator:
    ...
    def run_v10_backtest(self, df):
        ...
        try:
            ...
            print(f"V10简化信号生成完成: {signals_df.shape}")
            ...
            if signal_count > 0:
                ...
                if not trades_df.empty:
                    print(f"V10简化策略回测完成: {len(trades_df)}笔交易")
                ...
                print("V10简化信号数量为0，跳过策略回测")
                return None, None, signals_df
                
        except Exception as e:
            print(f"V10简化信号回测失败: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _generate_simple_signals(self, df):
        ...
        signals_df['model_type'] = 'v10_simple'
        
        # 基于OFI Z-score生成信号
        if 'ofi_z' in signals_df.columns:
            # 多头信号
            long_mask = signals_df['ofi_z'] > 2.0
            ...
            # 空头信号
            short_mask = signals_df['ofi_z'] < -2.0
...
def main():
    """主函数"""
    print("V10.0 实时模拟器 - 10ms步进 + 简化V10集成")
    print("="*60)
