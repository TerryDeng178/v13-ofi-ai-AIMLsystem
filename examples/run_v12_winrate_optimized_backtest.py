...
class V12WinRateOptimizedBacktest:
    """
    V12胜率优化版回测系统
    ...
    def __init__(self):
        ...
        # 胜率优化参数
        self.min_confidence = 0.5  # 从0.4提升到0.5，提高信号质量
        self.signal_strength_threshold = 0.4  # 从0.3提升到0.4，过滤弱信号
        
        # 市场状态专用参数
        self.market_state_params = {
            'trending': {
                'confidence_threshold': 0.6,  # 趋势市场提高置信度要求
                'signal_threshold': 0.5,
                'trend_confirmation_bars': 3,  # 需要3根K线确认趋势
                'trend_strength_min': 0.4
            },
            'high_volatility': {
                'confidence_threshold': 0.45,  # 高波动市场降低要求
                'signal_threshold': 0.35,
                'quick_exit_threshold': 0.8  # 快速止盈
            },
            'low_volatility': {
                'confidence_threshold': 0.55,
                'signal_threshold': 0.45,
                'patience_factor': 1.2  # 耐心等待更好机会
            },
            'ranging': {
                'confidence_threshold': 0.6,
                'signal_threshold': 0.5,
                'mean_reversion_factor': 0.8  # 均值回归策略
            }
        }
        
        # 风险管理参数
        self.max_position_size = 0.8  # 从1.0降低到0.8
        self.stop_loss_threshold = 0.015  # 从0.02降低到0.015
        self.take_profit_threshold = 0.012  # 从0.015降低到0.012
        self.dynamic_sizing = True  # 启用动态仓位管理
        
        # 信号质量控制
        self.signal_quality_filters = {
            'min_ofi_strength': 1.5,  # OFI强度最低要求
            'min_cvd_strength': 1.2,  # CVD强度最低要求
            'max_spread_bps': 8.0,    # 最大点差限制
            'min_volume_ratio': 1.1   # 最小成交量比例
        }
        ...
        logger.info("V12胜率优化版回测系统初始化完成")
        logger.info(f"胜率优化参数: 置信度阈值={self.min_confidence}, 信号强度阈值={self.signal_strength_threshold}")
        logger.info("市场状态专用参数已加载")
    ...
    def generate_enhanced_market_data(self, duration_hours: int = 24) -> List[Dict[str, Any]]:
        ...
        for minute in range(total_minutes):
            ...
            if 0 <= hour < 6:  # 夜间：低波动
                ...
                # 添加趋势成分
                trend_component = np.sin(minute * 0.1) * 0.001
                ...
                # 添加震荡成分
                range_component = np.sin(minute * 0.2) * 0.0008
            ...
            # 计算趋势强度
            trend_strength = self._calculate_trend_strength(minute, total_minutes, market_state)
            
            # 计算动量指标
            momentum_indicators = self._calculate_momentum_indicators(minute, total_minutes)
    ...
    def _calculate_trend_strength(self, minute: int, total_minutes: int, market_state: str) -> float:
        """计算趋势强度"""
        progress = minute / total_minutes
        
        if market_state == 'trending':
            # 趋势市场：周期性强趋势
            base_trend = np.sin(progress * 6 * np.pi) * 0.8
        elif market_state == 'high_volatility':
            # 高波动市场：随机性强趋势
            base_trend = np.random.normal(0, 0.6)
        elif market_state == 'low_volatility':
            # 低波动市场：弱趋势
            base_trend = np.sin(progress * 2 * np.pi) * 0.3
        else:  # ranging
            # 震荡市场：均值回归
            base_trend = np.sin(progress * 8 * np.pi) * 0.4
        
        noise = np.random.normal(0, 0.1)
        return base_trend + noise
    
    def _calculate_momentum_indicators(self, minute: int, total_minutes: int) -> Dict[str, float]:
        """计算动量指标"""
        return {
            'price_momentum_5m': np.random.normal(0, 0.5),
            'price_momentum_10m': np.random.normal(0, 0.4),
            'volume_momentum': np.random.normal(0, 0.3),
            'volatility_momentum': np.random.normal(0, 0.2)
        }
    ...
    def calculate_enhanced_ofi_features(self, data_point: Dict[str, Any], history: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        try:
            ...
            if len(history) >= 10:
                ...
                price_momentum_10m = (recent_prices[-1] - recent_prices[0]) / recent_prices[0]
                ofi_features['price_momentum_10m'] = price_momentum_10m
                ...
                # 计算趋势确认
                trend_confirmation = self._calculate_trend_confirmation(history[-10:])
                ofi_features['trend_confirmation'] = trend_confirmation
            
            # 计算信号质量分数
            signal_quality = self._calculate_signal_quality(ofi_features)
    ...
    def _calculate_trend_confirmation(self, history: List[Dict[str, Any]]) -> float:
        """计算趋势确认强度"""
        if len(history) < 3:
            return 0.0
        ...
        # 简单趋势确认：连续上涨或下跌
        if prices[2] > prices[1] > prices[0]:  # 上涨趋势
            return 1.0
        elif prices[2] < prices[1] < prices[0]:  # 下跌趋势
            return -1.0
        else:  # 无明确趋势
            return 0.0
    
    def _calculate_signal_quality(self, ofi_features: Dict[str, float]) -> float:
        """计算信号质量分数"""
        try:
            quality_score = 0.0
            ...
            if volume >= 5000:  # 最小成交量要求
                quality_score += 0.2
            
            # RSI评分
            rsi = ofi_features.get('rsi', 50.0)
            if 30 <= rsi <= 70:  # 避免极端RSI
                quality_score += 0.1
    ...
    def generate_market_state_optimized_signal(self, ofi_features: Dict[str, Any], market_data: Dict[str, Any]) -> Dict[str, Any]:
        """生成市场状态优化的信号"""
        try:
            ...
            if market_state == 'trending':
                ...
                trend_signal = np.tanh(
                    (ofi_z * 0.3 + cvd_z * 0.2 + trend_strength * 0.3 + trend_confirmation * 0.2)
                )
                trend_confidence = min(1.0, abs(trend_signal) + 0.4 + trend_strength * 0.3)
                
                return {
                    'ofi_signal': trend_signal,
                    'ofi_confidence': trend_confidence,
                    'ai_signal': trend_signal * 0.9,
                    'ai_confidence': trend_confidence * 0.9,
                ...
                volatility_signal = np.tanh(
                    (ofi_z * 0.4 + cvd_z * 0.3 + ofi_features.get('ofi_momentum_1s', 0.0) * 0.3)
                )
                volatility_confidence = min(1.0, abs(volatility_signal) + 0.3)
                
                return {
                    'ofi_signal': volatility_signal,
                    'ofi_confidence': volatility_confidence,
                    'ai_signal': volatility_signal * 0.8,
                    'ai_confidence': volatility_confidence * 0.8,
                ...
                low_vol_signal = np.tanh(
                    (ofi_z * 0.5 + cvd_z * 0.3 + trend_strength * 0.2) * patience_factor
                )
                low_vol_confidence = min(1.0, abs(low_vol_signal) + 0.5)
                
                return {
                    'ofi_signal': low_vol_signal,
                    'ofi_confidence': low_vol_confidence,
                    'ai_signal': low_vol_signal * 0.7,
                    'ai_confidence': low_vol_confidence * 0.7,
                ...
                ranging_signal = np.tanh(
                    (ofi_z * 0.4 + cvd_z * 0.3 + ofi_features.get('price_momentum_10m', 0.0) * 0.3) * mean_reversion_factor
                )
                ranging_confidence = min(1.0, abs(ranging_signal) + 0.4)
                
                return {
                    'ofi_signal': ranging_signal,
                    'ofi_confidence': ranging_confidence,
                    'ai_signal': ranging_signal * 0.8,
                    'ai_confidence': ranging_confidence * 0.8,
        ...
        try:
            ...
            if confidence >= state_confidence_threshold and abs(signal_strength) >= dynamic_threshold:
                ...
                if self.dynamic_sizing:
                    ...
                    quantity = base_size * strength_factor * confidence_factor * quality_factor
    ...
    def execute_enhanced_trade(self, trade_signal: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 根据信号质量调整滑点
            quality_slippage_factor = max(0.7, 1.0 - signal_quality * 0.3)
            
            slippage_bps = np.random.uniform(0.1, base_slippage) * slippage_multiplier * quality_slippage_factor
            ...
            # 基础价格变化
            base_change = np.random.normal(0, 0.002)  # 降低波动性
            
            # 根据信号质量调整预期收益
            quality_bonus = signal_quality * 0.01  # 信号质量奖励
            expected_return = signal_strength * confidence * 0.008 + quality_bonus  # 降低基准收益
            ...
            # 动态止损止盈
            dynamic_stop_loss = self.stop_loss_threshold * (1 + signal_quality * 0.5)  # 高质量信号放宽止损
            dynamic_take_profit = self.take_profit_threshold * (1 + signal_quality * 0.3)  # 高质量信号提高止盈
    ...
    def run_winrate_optimized_backtest(self, duration_hours: int = 24) -> Dict[str, Any]:
        """运行胜率优化版回测"""
        logger.info("=" * 80)
        logger.info("V12胜率优化版回测开始")
        logger.info("=" * 80)
        ...
        try:
            ...
            for i, data_point in enumerate(market_data):
                try:
                    ...
                    # 2. 生成市场状态优化信号
                    ai_signal = self.generate_market_state_optimized_signal(ofi_features, data_point)
                    ...
                    if trade_signal['action'] == 'rejected':
                        rejected_signals += 1
                    elif trade_signal.get('signal_quality', 0.0) < 0.6:
                    ...
                    if trade_record:
                        ...
                        logger.info(f"交易 {self.total_trades}: {trade_signal['side']} "
                                   ...
                                   f"PnL: {trade_record['pnl']:.2f}{stop_loss_msg}{take_profit_msg}{quality_msg}")
            ...
            # 生成回测报告
            backtest_report = self._generate_winrate_optimized_report(duration_hours, rejected_signals, low_quality_signals)
            
            logger.info("=" * 80)
            logger.info("V12胜率优化版回测完成")
            logger.info("=" * 80)
            
            return backtest_report
            
        except Exception as e:
            logger.error(f"胜率优化回测失败: {e}")
            return {'error': str(e)}
    ...
    def _generate_winrate_optimized_report(self, duration_hours: int, rejected_signals: int, low_quality_signals: int) -> Dict[str, Any]:
        """生成胜率优化回测报告"""
        try:
            # 计算回测时长
            ...
            optimization_analysis = {
                'parameter_changes': {
                    'confidence_threshold': f"{0.4} -> {self.min_confidence}",
                    'signal_strength_threshold': f"{0.3} -> {self.signal_strength_threshold}",
                    'signal_quality_filter': "新增信号质量过滤",
                    'market_state_params': "新增市场状态专用参数",
                    'dynamic_position_sizing': self.dynamic_sizing,
                    'enhanced_stop_loss': "动态止损止盈"
                },
            ...
            report = {
                ...
                'summary': {
                    ...
                    'sharpe_ratio': self.sharpe_ratio,
                    'max_drawdown': self.max_drawdown,
                    'winrate_optimization_success': {
                        'frequency_maintained': daily_trade_frequency >= self.target_daily_trades * 0.5,
                        'win_rate_improved': self.performance_metrics.get('win_rate', 0.0) > 0.5,
            ...
            logger.error(f"生成胜率优化回测报告失败: {e}")
            return {'error': str(e)}

def main():
    """主函数"""
    logger.info("=" * 80)
    logger.info("V12胜率优化版回测系统启动")
    logger.info("=" * 80)
    
    try:
        # 创建胜率优化版回测系统
        backtest_system = V12WinRateOptimizedBacktest()
        
        # 运行胜率优化版回测
        report = backtest_system.run_winrate_optimized_backtest(duration_hours=24)
        
        # 保存报告
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        report_file = f"v12_winrate_optimized_backtest_report_{timestamp}.json"
        
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(report, f, indent=2, ensure_ascii=False, default=str)
        
        logger.info(f"胜率优化回测报告已保存: {report_file}")
        
        # 打印摘要
        if 'summary' in report:
            summary = report['summary']
            targets_met = summary.get('targets_met', {})
            optimization_success = summary.get('winrate_optimization_success', {})
            
            logger.info("=" * 80)
            logger.info("V12胜率优化版回测摘要:")
