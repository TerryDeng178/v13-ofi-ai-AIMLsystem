...
from strategy_v8 import run_strategy_v8_intelligent, run_strategy_v8_advanced, run_strategy_v8_multi_signal
from backtest import calculate_trade_metrics
from risk_v8 import calculate_profitability_metrics

def main():
    import argparse
    parser = argparse.ArgumentParser(description="Run v8 strategy backtest.")
    parser.add_argument("--config", type=str, default="config/params_v8_profitability.yaml",
                        help="Path to the YAML configuration file.")
    parser.add_argument("--signal-type", type=str, default="intelligent_filter",
                        choices=["intelligent_filter", "frequency_optimized"],
                        help="Type of signal to use.")
    parser.add_argument("--strategy-mode", type=str, default="ultra_profitable",
                        choices=["ultra_profitable", "frequency_optimized"],
                        help="Strategy mode.")
    ...
    if args.multi_signal:
        print("运行多信号对比...")
        results = run_strategy_v8_multi_signal(df, params)
        ...
        for name, trades in results.items():
            if not trades.empty:
                ...
                comparison_data[name] = {
                    ...
                    "avg_intelligence_score": trades["intelligence_score"].mean() if "intelligence_score" in trades.columns else 0.0
                }
                print(f"  {name} 结果:")
                ...
                print(f"    平均智能评分: {comparison_data[name]['avg_intelligence_score']:.3f}")
            else:
                comparison_data[name] = {
                    "total_trades": 0, "win_rate": 0.0, "total_pnl": 0.0, "net_pnl": 0.0,
                    "cost_efficiency": 0.0, "profitability_score": 0.0, "profit_factor": 0.0,
                    "avg_signal_strength": 0.0, "avg_quality_score": 0.0, "avg_intelligence_score": 0.0
                }
                print(f"  {name} 结果: 无交易")

        comparison_path = os.path.join(output_dir, "v8_multi_signal_comparison.json")
        ...
        # Run single strategy
        print("运行策略...")
        trades = run_strategy_v8_intelligent(df, params, args.signal_type, args.strategy_mode)

        # Save trades
        trades_path = os.path.join(output_dir, "trades_v8.csv")
        ...
        print(f"平均智能评分: {summary['performance_summary'].get('avg_intelligence_score', 0.0):.3f}")
