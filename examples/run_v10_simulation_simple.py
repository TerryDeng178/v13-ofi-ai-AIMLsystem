...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'eth_perp_ofi_sim_realtime'))

# 导入独立V10模拟器
try:
    from eth_perp_ofi_sim_realtime.test_v10_standalone import V10StandaloneOFI, SimpleMarketSimulator
    print("使用独立V10模拟器")
except ImportError:
    print("无法导入V10模拟器，使用内置版本")
...
def simulate_trading(df_market, df_signals, initial_equity=100000):
    """模拟交易"""
    print("模拟交易执行...")
...
def run_v10_simulation_backtest(df_market, df_ofi, df_signals, test_id):
    """运行V10模拟器回测"""
    print("\n" + "="*60)
    ...
    # 运行交易模拟
    trades_df = simulate_trading(df_market, df_signals, 100000)
...
def create_test_report(test_id, results, trades_df, df_market, df_ofi, df_signals):
    """创建测试报告"""
    ...
    # 创建报告
    report_content = f"""# V10.0 模拟器回测报告 - 测试{test_id}
...
"""
        ...
        if results['avg_quality'] > 0.8:
            report_content += "- ✅ **优秀**: 平均质量 > 0.8，信号质量高\n"
        elif results['avg_quality'] > 0.6:
            report_content += "- ⚠️ **一般**: 平均质量 > 0.6，信号质量一般\n"
        else:
            report_content += "- ❌ **不佳**: 平均质量 < 0.6，信号质量需要改进\n"
...
def main():
    ...
    for test_id in range(1, 6):  # 运行5次测试
        ...
        try:
            ...
            results, trades_df = run_v10_simulation_backtest(
