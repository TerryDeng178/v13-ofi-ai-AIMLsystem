#!/usr/bin/env python3
"""
V10.0 模拟器数据生成和深度学习回测
使用V10模拟器生成市场数据，然后进行深度学习回测
"""
...
try:
    ...
    print(f"模拟器导入错误: {e}")
    # 使用独立版本
    from test_v10_standalone import V10StandaloneOFI, SimpleMarketSimulator
...
def run_v10_deep_learning_backtest(df_market, df_ofi, df_signals, test_id):
    """运行V10深度学习回测"""
    print("\n" + "="*60)
    print(f"V10.0 深度学习回测 - 测试{test_id}")
    print("="*60)
...
def create_test_report(test_id, results, trades_df, signals_df, df_market, df_ofi, df_signals):
    """创建测试报告"""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    report_dir = f"test_reports/test_{test_id}_{timestamp}"
    ...
    # 创建报告
    report_content = f"""# V10.0 深度学习回测报告 - 测试{test_id}
...
"""
        ...
        if results['roi'] > 5:
            report_content += "- ✅ **优秀**: ROI > 5%，盈利能力强劲\n"
        elif results['roi'] > 0:
            report_content += "- ⚠️ **一般**: ROI > 0%，有盈利但需要优化\n"
        else:
            report_content += "- ❌ **不佳**: ROI < 0%，需要大幅改进\n"
...
"""
        ...
        if results['max_drawdown'] < 5:
            report_content += "- ✅ **优秀**: 最大回撤 < 5%，风险控制良好\n"
        elif results['max_drawdown'] < 10:
            report_content += "- ⚠️ **一般**: 最大回撤 < 10%，风险控制一般\n"
        else:
            report_content += "- ❌ **不佳**: 最大回撤 > 10%，风险控制需要改进\n"
...
"""
        ...
        if results['avg_quality'] > 0.8:
            report_content += "- ✅ **优秀**: 平均质量评分 > 0.8，信号质量高\n"
        elif results['avg_quality'] > 0.6:
            report_content += "- ⚠️ **一般**: 平均质量评分 > 0.6，信号质量一般\n"
        else:
            report_content += "- ❌ **不佳**: 平均质量评分 < 0.6，信号质量需要改进\n"
...
# 建议的优化参数
risk:
  max_trade_risk_pct: {max(0.005, results['max_trade_risk_pct'] * 0.8)}  # 降低风险
  atr_stop_lo: {max(0.04, results.get('atr_stop_lo', 0.06) * 0.8)}  # 收紧止损
  atr_stop_hi: {min(3.0, results.get('atr_stop_hi', 2.5) * 1.2)}  # 提高止盈

signals:
  ofi_z_min: {max(1.0, results.get('ofi_z_min', 1.2) * 0.9)}  # 降低OFI阈值
  min_signal_strength: {max(1.2, results.get('min_signal_strength', 1.6) * 0.9)}  # 降低强度要求
  min_confidence: {max(0.6, results.get('min_confidence', 0.8) * 0.9)}  # 降低置信度要求

sizing:
  k_ofi: {min(1.0, results.get('k_ofi', 0.7) * 1.2)}  # 提高仓位倍数
  size_max_usd: {min(500000, results.get('size_max_usd', 300000) * 1.2)}  # 提高最大仓位
```
...
def main():
    ...
    for test_id in range(1, 6):  # 运行5次测试
        ...
        try:
            ...
            results, trades_df, signals_df = run_v10_deep_learning_backtest(
                df_market, df_ofi, df_signals, test_id
            )
            
            # 3. 创建测试报告
            print(f"\n步骤3: 创建测试报告 (测试{test_id})")
            report_dir = create_test_report(
                test_id, results, trades_df, signals_df, 
                df_market, df_ofi, df_signals
            )
    ...
    print("V10.0 模拟器测试完成")
    print(f"{'='*60}")
    print("所有测试报告已保存到 test_reports/ 目录")
